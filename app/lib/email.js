...
class EmailSender {
  ...
  please use the link below to comfirm your email <a href="${process.env.HOST}/auth/comfirm?token=${token}">${process.env.HOST}/comfirm?token=${token}</a>`, // html body
    };
...
transport.sendMail(mail, (err, info) => {
  ...
  resetLink = (user, name, token) => {
    ...
    please click the button to reset your password <a href='${
      process.env.HOST
    }/resetPassword?token=${token}'>reset Password</a>`, // plain text body
